program MAIN
  ...
  integer :: i,j,ups,downs     ! counters for loops
  integer :: iconfig
  integer :: n
  ...
  allocate(pars%Ei(pars%nx*pars%ny))
  read(*,*) n_up, n_dn, pars%Ei
  !write(*,*) pars%Ei
  call evaluate_parameters(n_up,n_dn)                                ! in the parameter module found in parameters.f90 will calculate the dependant parameters
  ...
  do iconfig = 1,pars%iconfig
     ...
     write(999,'("Energy:  ",g16.9)') GroundStateEnergy - pars%mu*n
