...
! FINISHED code. not very touched up or pretty will work on that for later versions
...
real, allocatable, dimension(:) :: GIPR_num, GIPR_den, GIPR
real :: sum
integer :: bin
real :: E(nsites)
...
allocate(GIPR(nbins))
allocate(GIPR_num(nbins))
allocate(GIPR_den(nbins))
do j=1,npairs
E = 0
eigenvector = 0
GIPR = 0
