...
do i=1,128
   ...
   if ((LDOS(1,i,2) + LDOS(2,i,2) + LDOS(3,i,2)) /= 0) then
      ...
      GIPR_num(bin) = GIPR_num(bin) + IPR(i)*(LDOS(1,i,2) + LDOS(2,i,2) + LDOS(3,i,2))/3.0 !numerator of the wieghted GIPR
      GIPR_den(bin) = GIPR_den(bin) + (LDOS(1,i,2) + LDOS(2,i,2) + LDOS(3,i,2))/3.0  !denominator of the wieghted GIPR
   end if
end do
...
do i=2,nbins                                    ! calculate sum to normalize it to 1
